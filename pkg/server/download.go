...
func (s *server) Download(ctx context.Context, req *api.DownloadRequest) (*api.DownloadResponse, error) {
	...
	ss := req.Type
	if ss == api.DownloadRequest_PACKAGE {
		...
		if requestData.GetPresignedUrl {
			...
			go func() {
				...
				responseType = api.DownloadResponse_DOWNLOAD
				ctx, cancelFnc := context.WithCancel(context.Background())
				session := downloadSession{
					id:        requestData.PackageId,
					cancelFnc: cancelFnc,
				}

				s.downloadCancelFncs.Store(requestData.PackageId, session)
