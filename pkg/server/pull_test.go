...
func TestFindMappedDatasetRoot(t *testing.T) {
	...
	root, found, err = findMappedDatasetRoot(filepath.Join("..", "..", "resources", "test", "pullTest"))
	assert.NoError(t, err)
	assert.True(t, found,
	...
	root, found, err = findMappedDatasetRoot(filepath.Join("..", "..", "resources", "test", "pullTest", "folder_1", "folder_2", "file_4.txt"))
	assert.NoError(t, err)
	assert.True(t, found,
