...
func TestFindMappedDatasetRoot(t *testing.T) {
	...
	root, found, err := findMappedDatasetRoot(path.Join("..", "..", "resources", "test", "pullTest", "folder_1", "folder_2"))
	...
	root, found, err = findMappedDatasetRoot(path.Join("..", "..", "resources", "test", "pullTest"))
	assert.NoError(t, err)
	assert.True(t, found,
		"Should find the manifest file in root folder when starting at root")

	root, found, err = findMappedDatasetRoot(path.Join("..", "..", "resources"))
	assert.NoError(t, err)
	assert.False(t, found,
		"Should not find a root if not starting in mapped dataset")

	root, found, err = findMappedDatasetRoot(path.Join("..", "..", "resources", "test", "pullTest", "folder_1", "folder_2", "file_4.txt"))
	assert.NoError(t, err)
	assert.True(t, found,
