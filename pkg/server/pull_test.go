...
func TestFindMappedDatasetRoot(t *testing.T) {
	...
	root, found, err := findMappedDatasetRoot(filepath.Join("..", "..", "test", "testData", "pullTest", "folder_1", "folder_2"))
	...
	root, found, err = findMappedDatasetRoot(filepath.Join("..", "..", "test", "testData", "pullTest"))
	assert.NoError(t, err)
	assert.True(t, found,
	...
	root, found, err = findMappedDatasetRoot(filepath.Join("..", "..", "test", "testData", "pullTest", "folder_1", "folder_2", "file_4.txt"))
	assert.NoError(t, err)
	assert.True(t, found,
