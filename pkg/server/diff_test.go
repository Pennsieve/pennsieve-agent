...
func TestCompareManifest(t *testing.T) {
	datasetRoot := "../../resources/test/mapDataset"
	files, _ := createFolderManifest(datasetRoot)

	manifest, err := shared.ReadWorkspaceManifest("../../resources/test/mapDataset/.pennsieve/manifest.json")
	...
	for _, f := range result {
		...
		case "folder_1/file_4_renamed.txt":
			...
			count++
		case "folder_1/folder_1_1/file_5_moved.txt":
		...
		case "folder_2/folder_2_1/file_12_downloaded_moved_renamed.txt":
			...
			count++
		case "folder_2/folder_2_1/file_13_moved_renamed.txt":
			assert.Equal(t, api.PackageStatus_MOVED_RENAMED, f.Type)
			...
			assert.Equal(t, f.Old.Path, "folder_2")
			count++
		case "folder_2/folder_2_1/file_8_downloaded_moved.txt":
			assert.Equal(t, api.PackageStatus_MOVED, f.Type)
			assert.Equal(t, f.New.Path, "folder_2/folder_2_1")
			assert.Equal(t, f.Old.Path, "folder_2")
			count++
		case "folder_2/file_9_downloaded_changed.txt":
